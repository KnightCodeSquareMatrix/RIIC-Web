...
- 导入 MAA `Arknights_OperBox_Export.json` 和兼容的一图流 XLSX / XLS；Box、布局、手动排班和后台复现文件均先打开上传弹窗，可拖入文件或点击“选择文件”，并显示支持格式和已有大小限制。选择后自动接续原流程，保留异常练度审阅、手动草稿覆盖确认和后台预览；失败或取消不会替换当前数据
