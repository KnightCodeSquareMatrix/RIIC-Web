...
test("plan task admission uses 1000 global slots, 600 new-account slots, and a bounded candidate pool", () => {
  ...
  assert.equal(PLAN_TASK_WORKER_CONCURRENCY, 2);
...
test("plan task ETA reflects two solver lanes", () => {
  assert.equal(planTaskEtaSeconds(1), 3);
  assert.equal(planTaskEtaSeconds(2), 3);
  assert.equal(planTaskEtaSeconds(3), 6);
});
