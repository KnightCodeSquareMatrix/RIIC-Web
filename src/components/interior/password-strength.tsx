...
export function PasswordStrength({ value, className = "" }: { value: string; className?: string }) {
  ...
  const state = useMemo(() => {
    const rules = DEFAULT_RULES.map((rule) => ({
      ...rule,
      label: en ? ({ length: "At least 10 characters", letter: "Contains a letter", digit: "Contains a number", variety: "Mixed case or a symbol" }[rule.id] ?? rule.label) : rule.label,
      met: rule.test(value),
    }));
    ...
    const announcement = value.length === 0
      ? ""
      : en
        ? `Password strength: ${EN_LABELS[score]}. ${guessable ? "This password pattern is easy to guess. " : ""}${unmet.length ? `Still needed: ${unmet.map((rule) => rule.label).join(", ")}.` : "All recommendations are met."}`
