      ...
      <InputGroupAddon align="inline-end">
        {showTrigger && (
          <InputGroupButton
            ...
            className="group-has-data-[slot=combobox-clear]/input-group:hidden max-sm:min-h-0 max-sm:min-w-0 data-pressed:bg-transparent"
            disabled={disabled}
          />
        )}
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List data-yeye-scroll="auto"
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-md py-1 pr-8 pl-1.5 text-sm outline-hidden select-none data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 max-sm:min-h-[44px] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
